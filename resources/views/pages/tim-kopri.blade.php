@extends('layouts.pages')
@section('title', 'KOPRI | PMII UNUSIA BOGOR')

@section('content')
    <!-- Start Team Area
    ============================================= -->
    <div id="team" class="team-area radius-less default-padding bottom-less">
        <div class="container">
            <div class="row">
                <div class="col-lg-8 col-md-8 col-lg-offset-2 col-md-offset-2">
                    <div class="site-heading text-center">
                        <h2> KOPRI PMII UNUSIA BOGOR <br /> Masa Khidmat 2024-2025 </h2><br/>
                        <a class="ms-5 btn btn-primary p-3" href="{{ route('tim.index') }}">KOMISARIAT UNUSIA</a>
                        <a class="btn btn-primary p-3" href="{{ route('tim-kopri') }}">KOPRI UNUSIA </a>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="team-items text-center">
                    @foreach($kopri_pengurus as $pengurus)
                        <div class="col-md-4 single-item">
                            <div class="item">
                                <div class="thumb">
                                    <img src="{{ $pengurus->image_url }}" alt="Thumb" />
                                    <ul>
                                        <li class="facebook">
                                            <a href="{{ $pengurus->facebook }}">
                                                <i class="fab fa-facebook-f"></i>
                                            </a>
                                        </li>
                                        <li class="twitter">
                                            <a href="{{ $pengurus->twitter }}">
                                                <i class="fab fa-twitter"></i>
                                            </a>
                                        </li>
                                        <li class="instagram">
                                            <a href="{{ $pengurus->instagram }}">
                                                <i class="fab fa-instagram"></i>
                                            </a>
                                        </li>
                                        <li class="linkedin">
                                            <a href="{{ $pengurus->linkedin }}">
                                                <i class="fab fa-linkedin-in"></i>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="info">
                                    <h4>{{ $pengurus->nama_lengkap }}</h4>
                                    <span>{{ $pengurus->jabatan }}</span>
                                    <h5>
                                        <i class="fas fa-envelope"></i> {{ $pengurus->email }}
                                    </h5>
                                </div>
                            </div>
                        </div>
                    @endforeach
                </div>
            </div>
        </div>
    </div>
    <!-- End Team Area -->
@endsection
