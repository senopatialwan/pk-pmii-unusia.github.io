                ...
                <tbody>
                  @foreach ($anggota_verifikasi as $anggota)
                    <tr>
                      ...
                      <td>
                          <span>{{ $anggota->status == 1 ? 'Diterima': ($anggota->status == 0 ? 'Belum ditentukan' : 'Ditolak') }}</span
                      </td>
                      <td>
                        <a href="{{route('admin.anggota.show')}}" class="btn btn-info"><i class="fa fa-eye"></i></a>
                        <form action="{{ route('admin.anggota.verifikasi-kta.handle') }}" method="POST" enctype="multipart/form-data">
                          @csrf
                          <input type="hidden" name="id" value="{{ $anggota->id }}">
                          <button type="submit" name="status" value="1" class="btn btn-success"><i class="fa fa-check"></i></button>
                          <button type="submit" name="status" value="2" class="btn btn-orange"><i class="fa fa-close"></i></button>
